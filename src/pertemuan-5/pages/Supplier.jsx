...
import { listSuppliers, createSupplier, updateSupplier, deleteSupplier } from '@/lib/apiSupplier';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Edit, Trash2, X, Save, Plus } from 'lucide-react';

const EMPTY = { nama: '', kontak: '', email: '', alamat: '' };
...
export default function Supplier() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        ...
        setSuccessMsg('Data supplier berhasil diperbarui.');
      } else {
        await createSupplier(form);
        setSuccessMsg('Supplier baru berhasil ditambahkan.');
      }
      setForm(EMPTY);
      setEditingId(null);
      setShowForm(false);
  ...
  const handleEdit = (s) => {
    setForm({ nama: s.nama, kontak: s.kontak, email: s.email, alamat: s.alamat });
    setEditingId(s.id);
    setShowForm(true);
    setErrorMsg('');
    setSuccessMsg('');
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-sm border border-slate-200/70 shadow-sm p-6">
        <div className="absolute inset-0 bg-[radial-gradient(closest-side,rgba(139,92,246,0.12),transparent)] opacity-60" />
        <div className="relative flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-800 tracking-tight">Kelola Supplier</h2>
            <p className="text-slate-500 text-sm">Manajemen data supplier obat dan barang untuk apotek.</p>
          </div>
          {!showForm && (
          <button
      ...
      {/* Form */}
      {showForm && (
        <Card className="border-violet-200 bg-violet-50/20 shadow-md">
          <CardHeader className="border-b border-violet-100 flex flex-row items-center justify-between">
            <div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2.5 border border-slate-200 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">No. Kontak</label>
                <input
                  name="kontak" value={form.kontak} onChange={handleChange} required
      ...
      <Card className="border-slate-200/80 shadow-sm overflow-hidden">
        <CardHeader className="bg-slate-50/70 border-b border-slate-100 flex flex-row items-center justify-between">
          <div>
            <CardTitle className="text-base font-bold text-slate-800 flex items-center gap-2">
              <span>🏭</span> Daftar Supplier
            </CardTitle>
            <CardDescription>Semua supplier terdaftar di sistem</CardDescription>
          </div>
          <div className="text-xs font-bold text-slate-400 uppercase bg-white border px-3 py-1 rounded-xl shadow-inner">
            {loading ? 'Memuat...' : `${suppliers.length} Supplier`}
          </div>
        </CardHeader>
              ...
              <tbody className="divide-y divide-slate-100">
                {suppliers.map((s, i) => (
                  <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 font-semibold text-slate-800 whitespace-nowrap">{s.nama}</td>
                    <td className="py-4 px-6 text-slate-500">{s.kontak}</td>
                    <td className="py-4 px-6 text-slate-500">{s.email}</td>
                    <td className="py-4 px-6 text-slate-500">{s.alamat}</td>
