...
import { processPosSale } from '@/lib/apiPos';
...
export default function PointOfSales() {
  ...
  useEffect(() => {
    (async () => {
      try { setProducts(await listProducts()); } catch (e) { setErrorMsg(e.message); }
    })();
  }, []);

  const filteredProducts = useMemo(
    () => products.filter((p) => p.name.toLowerCase().includes(searchQuery.toLowerCase()) && p.stock > 0),
    ...
    () =>
      Object.entries(cart)
        ...
        .map(([productId, quantity]) => {
          ...
          return product ? { productId, quantity, price: product.price, name: product.name } : null;
        })
        .filter(Boolean),
    [cart, products]
  );
  ...
  async function handleCheckout() {
    ...
    setSuccessMsg('');
    try {
      const result = await processPosSale({ items: cartItems, adminId: session?.user?.id });
      setCart({});
      setSuccessMsg(`Transaksi berhasil! Total: Rp ${Number(result.totalAmount).toLocaleString('id-ID')}`);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">Proses penjualan langsung di kasir apotek.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white/70 border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-4 border-b bg-slate-50/70 flex items-center justify-between gap-4">
            <span className="font-bold text-slate-700">Pilih Produk</span>
        ...
        <div className="bg-white/70 border border-slate-200 rounded-2xl shadow-sm p-5">
          <h3 className="font-bold text-slate-800 mb-4">Keranjang POS</h3>
          {cartItems.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-8">Belum ada item.</p>
          ) : (
